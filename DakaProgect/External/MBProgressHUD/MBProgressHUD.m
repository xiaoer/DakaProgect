...
//custom view for cancel button
+ (MBProgressHUD *)showHUDAddedToWithCancelButton:(UIView *)view animated:(BOOL)animated
{
    MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:view];
    
    UIView *customView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 90, 100)];
    ...
    [cancelRequestButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [cancelRequestButton setTitle:@"Cancel" forState:UIControlStateNormal];
    [cancelRequestButton.titleLabel setFont:[UIFont systemFontOfSize:14]];
    [cancelRequestButton setBackgroundImage:[[UIImage imageNamed:@"x3.png"] stretchableImageWithLeftCapWidth:21 topCapHeight:14] forState:UIControlStateNormal];
    cancelRequestButton.frame = CGRectMake(5, indicator.frame.origin.y+indicator.frame.size.height+16, 80, 25);
    [cancelRequestButton addTarget:hud action:@selector(cancelRequest:) forControlEvents:UIControlEventTouchUpInside];
    
    [customView addSubview:indicator];
    
    [customView addSubview:cancelRequestButton];
...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view text:(NSString *)text animated:(BOOL)animated
{
    ...
    CGSize size = [text sizeWithFont:font constrainedToSize:CGSizeMake(450.0f, 30.0f) lineBreakMode:NSLineBreakByCharWrapping];
    size.width += 20;
    size.height += 15;
